...
import { Modal, Button, Form, Input, Icon, message, Switch, DatePicker } from 'antd'
...
class UserModal extends React.Component {
  ...
  renderDatePick = () => {
    const {onRangePickerChange} = this
    const {password} = this.state.user
    return (
      <FormItem  {...formItemLayout} label="会员"
                 validateStatus={password.status} help={password.msg}>
        <RangePicker size='large' onChange={onRangePickerChange}/>
      </FormItem>
    )
  }

  render () {
    const {handleSubmit, onChange, renderIcon, onSwitchChange, renderDatePick} = this
      ...
      <Modal title="添加新用户" className="admin-user__model" visible={visible} width="50%"
        ...
        <div className="form-box flex-center">
          <Form className="user-modal__form">
            ...
            <FormItem  {...formItemLayout} label="微信" hasFeedback required
                       validateStatus={email.status} help={email.msg}>
              <Input size='large' disabled prefix={renderIcon('paper-clip')} onChange={e => {onChange(e, 'email')}}
                     placeholder="微信" value={email.value}/>
            </FormItem>
            <FormItem  {...formItemLayout} label="手机" hasFeedback required
                       validateStatus={phone.status} help={phone.msg}>
              <Input size='large' prefix={renderIcon('phone')} onChange={e => {onChange(e, 'phone')}}
                     placeholder="手机" value={phone.value}/>
            </FormItem>
            <FormItem  {...formItemLayout} label="密码" hasFeedback required
                       validateStatus={password.status} help={password.msg}>
              <Input size='large' type="password" prefix={renderIcon('lock')} onChange={e => {onChange(e, 'password')}}
                     placeholder="密码" value={password.value}/>
            </FormItem>
            <FormItem  {...formItemLayout} label="会员" hasFeedback required
                       validateStatus={password.status} help={password.msg}>
              <Switch size='large' onChange={onSwitchChange}/>
            </FormItem>
            {this.state.isVip ? renderDatePick() : ''}
          </Form>
        </div>
